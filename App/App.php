...
define("Server","https://raw.githubusercontent.com/Zivin30/Student/main/");
define("Data","Data/");
...
Function Api_Bal($api_url){
	...
	$r = json_decode(file_get_contents($api_url."/status/?key=".$apikey),1);
    if(!$r["balance"]){
        print ApiError;
    }
    return $r["balance"];
...
Function Captcha($source,$api_url,$apikey, $sitekey, $pageurl,$delay){
    ...
    if(preg_match("/nopecha/",$api_url)){return captchaNop($source,$api_url,$apikey, $sitekey, $pageurl,$delay);}
}
...
Function captchaNop($source,$api_url,$apikey, $sitekey, $pageurl,$delay){
    if(preg_match("/g-recaptcha/" ,$source)){$r = json_decode(file_get_contents($api_url."/token?key=".$apikey."&method=recaptcha2&googlekey=".$sitekey."&url=".$pageurl."&json=1"),1);}
    if(preg_match("/recaptchav3/" ,$source)){$r = json_decode(file_get_contents($api_url."/token?key=".$apikey."&method=recaptcha3&googlekey=".$sitekey."&url=".$pageurl."&json=1"),1);}
    if(preg_match("/turnstile/",$source)){$r = json_decode(file_get_contents($api_url."/token?key=".$apikey."&method=turnstile&sitekey=".$sitekey."&pageurl=".$url."&json=1"),1);}
    $status = $r["status"];
        if($status == 0){ApiError;return 0;}
        $id = $r["request"];
        while(true){
            load();
            $r = json_decode(file_get_contents($api_url."/token?key=".$apikey."&id=".$id."&json=1"),1);
            $status = $r["status"];
            if($r["request"] == "CAPCHA_NOT_READY"){print rr;load();sleep($delay);print rr;continue;}
            if($status == 1){print rr;print bps_cap();return $r["request"];}
            return 0;
